# Schistosoma mansoni V10 genome: HiC analysis
...
```bash
# working dir
cd /nfs/users/nfs_s/sd21/lustre118_link/schistosoma_mansoni/V10/HIC
```

```bash
# HiC data
32442_4#1_1.fastq.gz
32442_4#1_2.fastq.gz

# from Alan
/lustre/scratch118/infgen/team133/alt/SCHISTO/PB_SR_allMl2016/filtered_subreads.fastq.gz

# convert to fasta
fastaq to_fasta filtered_subreads.fastq filtered_subreads.fasta


# V5 reference genome
cp ../../V5/smansoni_protasio2013_genome.fa .
...
# input required
export PREFIX=smansoni_V10
export REFERENCE=/nfs/users/nfs_s/sd21/lustre118_link/schistosoma_mansoni/V10/REF/SM_V10.fa
export HIC_READS_R1=/nfs/users/nfs_s/sd21/lustre118_link/schistosoma_mansoni/V10/HIC/32442_4#1_1.fastq.gz
export HIC_READS_R2=/nfs/users/nfs_s/sd21/lustre118_link/schistosoma_mansoni/V10/HIC/32442_4#1_2.fastq.gz
export PACBIO_READS=/nfs/users/nfs_s/sd21/lustre118_link/schistosoma_mansoni/V10/HIC/filtered_subreads.fasta
export TELOMERE_SEQ=TTAGGG
...
# bam to bed
bedtools bamtobed -i ${COVERAGE_DIR}/${PREFIX}.minimap.sorted.pri.bam | bedtools sort > ${COVERAGE_DIR}/${PREFIX}.geval.bed
...
bedtools coverage -a ${REPEATS_DIR}/REF.genome.10000bp.bed -b ${REPEATS_DIR}/${PREFIX}.repeats.bed | awk '{print $1, $2, $3, $5}' OFS="\t" | sort -k1,1 -k2,2n > ${REPEATS_DIR}/${PREFIX}.repeats.bedgraph
...
bedtools coverage -b ${TELOMERES_DIR}/telomere_dimer.bed -a ${TELOMERES_DIR}/REF.genome.10000bp.bed  | awk '{print $1, $2, $3, $5}' OFS="\t" | sort -k1,1 -k2,2n > ${TELOMERES_DIR}/${PREFIX}.telomere_coverage.bedgraph
...
func_finish() {
     ...
     # check if finished successfully
     if [[ -f ${REF_DIR}/COMPLETE_reference_files ]] && [[ -f ${MAPPING_DIR}/COMPLETE_hic_mapping ]] && [[ -f ${PRETEXT_DIR}/COMPLETE_pretext ]] && [[ -f ${HIGLASS_DIR}/COMPLETE_higlass ]] && [[ -f ${COVERAGE_DIR}/COMPLETE_coverage ]] && [[ -f ${GAPS_DIR}/COMPLETE_gaps ]] && [[ -f ${REPEATS_DIR}/COMPLETE_repeats ]]
     then
...
bsub -w "02_map_hic_${PREFIX}" -E 'test -e /nfs/users/nfs_s/sd21' -R "select[mem>40000] rusage[mem=40000]" -n 4 -M40000 -o ${LOG_FILES}/03_pretext.o -e ${LOG_FILES}/03_pretext.e -J 03_pretext_${PREFIX} func_pretext

bsub -w "02_map_hic_${PREFIX}" -E 'test -e /nfs/users/nfs_s/sd21' -R "select[mem>30000] rusage[mem=30000]" -n 4 -M30000 -o ${LOG_FILES}/04_higlass.o -e ${LOG_FILES}/04_higlass.e -J 04_higlass_${PREFIX} func_higlass
...
bsub -w "02_map_hic_${PREFIX}" -E 'test -e /nfs/users/nfs_s/sd21' -R "select[mem>30000] rusage[mem=30000]" -n 4 -M30000 -o ${LOG_FILES}/09_juicer.o -e ${LOG_FILES}/09_juicer.e -J 09_juicer_${PREFIX} func_juicer
...
## Sort intermediate file for visualisation
- the pipeline above sorts the chromosomes by length, which is useful for normal viewing on chromosomes
- for schisto, want the chromosomes in a specific order
     - Ch

```bash
cd ~/lustre118_link/schistosoma_mansoni/V10/HIC/hicpipe_smansoni_V10_out/09_JUICER

>Sm_V10.pre.chromosome-order.bed

cat pre.bed | awk '$3=="SM_V10_1" {print}' OFS="\t" >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_2" {print}' OFS="\t" >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_3" {print}' OFS="\t"  >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_4" {print}' OFS="\t"  >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_5" {print}' OFS="\t"  >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_6" {print}' OFS="\t"  >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_7" {print}' OFS="\t"  >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_Z" {print}' OFS="\t"  >> Sm_V10.pre.chromosome-order.bed
cat pre.bed | awk '$3=="SM_V10_WSR" {print}' OFS="\t" >> Sm_V10.pre.chromosome-order.bed
